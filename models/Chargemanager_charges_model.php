...
class Chargemanager_charges_model extends App_Model
{
    ...
    public function generate_individual_invoice($charge_id, $options = [])
    {
        try {
            ...
            $invoice_data = [
                ...
                'allowed_payment_modes' => $this->get_allowed_payment_modes_for_invoice(),
    ...
    /**
     * Get allowed payment modes for invoice
     * @return string Serialized array of payment mode IDs
     */
    private function get_allowed_payment_modes_for_invoice()
    {
        // Obter os payment modes do ChargeManager
        $this->load->model('chargemanager_model');
        $chargemanager_modes = $this->chargemanager_model->get_chargemanager_payment_modes();
        $allowed_modes = array_column($chargemanager_modes, 'id');

        // Se não há modes do ChargeManager, permitir todos os modes ativos para faturas
        if (empty($allowed_modes)) {
            $this->db->where('active', 1);
            $this->db->where('invoices_only', 1);
            $all_invoice_modes = $this->db->get(db_prefix() . 'payment_modes')->result();
            $allowed_modes = array_column($all_invoice_modes, 'id');
        }

        return serialize($allowed_modes);
    }
} 