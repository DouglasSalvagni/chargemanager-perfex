...
$(document).ready(function() {
    ...
    $("#generate-schema").on("click", function() {
        ...
        generateAutoSchema(frequency, installmentValue);
    });
    
    // Gerar schema automático
    function generateAutoSchema(frequency, installmentValue) {
    ...
    setTimeout(function() {
        ...
        // Garantir que o botão "Adicionar Cobrança" funcione em ambos os modos
        if ($("#schema_type").val() === "auto") {
            // Se estiver no modo automático, garantir que o botão de adicionar cobrança do modo manual também funcione
            $("#manual-schema-config").find("#add-charge").on("click", function() {
                addCharge();
                updateEntryChargeStatus();
                calculateTotals();
                updateSchemaJson();
